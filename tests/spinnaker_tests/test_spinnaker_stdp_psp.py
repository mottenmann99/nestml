...
class TestSpiNNakerSTDPPSP:
    ...
    def run_sim(self, pre_spike_times, simtime=50):
        ...
        pre_input = p.Population(1, p.SpikeSourceArray(spike_times=pre_spike_times), label="pre_input")
        post_neuron = p.Population(1, iaf_psc_exp_neuron_nestml(), label="post_neuron")

        stdp_model = stdp_synapse_nestml(weight=5.) # XXX: weight seems to be hard-coded to 0x50000? PSC peak is 5, so seems to match, but where is 0x50000 coming from?
